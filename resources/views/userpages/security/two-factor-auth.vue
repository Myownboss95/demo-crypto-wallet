  ...
  <div class="col-lg-4 col-md-4 m-auto mt-5">
    <div class="card shadow">
      <div class="card-body">
        ...
        >
          <div
            v-if="$page.props.auth.user.two_factor_status"
            class="text-center"
          >
          ...
          <div v-else>
            ...
            <form v-else>
              <div class="text-center px-3 py-2">
                <p>
                  Download the google authenticator app from App Store or Play
                  store, scan the qr code below
                </p>
                ...
                <div class="text-justify">
                  <p>
                    ...
                    as a setup key to create a secure link from authenticator to
                    your account.
                  </p>
                  <p>
                    Copy your recovery codes and store them securely as they can
                    be used to disable Two Factor Authentication.
                  </p>
                  <div class="text-center">
                    <FormButton
                      class="btn btn-outline-success mb-3"
                      @button-clicked="() => (modalOpen = true)"
                    >
                      <ButtonLoader text="Recovery Codes" />
                    </FormButton>
                  </div>
                  <p>
                    After set up click on the confirm status button below to
                    enter code from the authenticator app to verify the setup.
                  </p>
                </div>
    ...
    <Modal
      id="ConfirmTwoFactorModal"
      size="sm"
      title="Confirm two factor auth"
      :open="twoFactorAuthModal"
      @modeal-closed="() => (twoFactorAuthModal = false)"
    >
      <div class="modal-body">
        <form class="pb-3" @submit.prevent="sendConfirmTwoFactorRequest">
          <p class="text-center">
            Enter the TOTP from the google authenticator app and click the
            confirm button
          </p>
          <FormGroup
            placeholder="TOTP code"
            v-model="confirmTwoFactorAuth.code"
          />
          <FormButton class="btn btn-outline-success w-100" type="submit">
            <ButtonLoader
              text="Confirm"
              :loading="sendConfirmTwoFactorRequest.processing"
            />
          </FormButton>
        </form>
      </div>
