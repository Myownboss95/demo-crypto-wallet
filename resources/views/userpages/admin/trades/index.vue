        ...
        <table class="table">
          ...
          <tbody v-if="trades.length">
            <tr v-for="(trade, key) in trades" :key="key">
              ...
              <td
                :class="{
                  'text-danger': trade.returns < 0,
                  'text-success': trade.returns > 0,
                }"
              >
              ...
              <td>
                <div v-if="trade.status == 'active'">
                  <FormButton
                    ...
                    @button-clicked="view(trade.id)"
                  >
                    <ButtonLoader text="<i class='fa fa-edit'></i>" />
                  </FormButton>

                  <FormButton
                    @button-clicked="closeTrade(trade.id)"
                    class="btn btn-outline-danger btn-sm ms-2"
                  >
                    <ButtonLoader
                      text="<i class='fa fa-times'></i>  Close"
                      :loading="form.processeing"
                    />
                  </FormButton>
                </div>
                <div else>--</div>
