<template>
  ...
  <breadcrumb title="Transfers" :crumbs="['Dashboard', 'Transfers']" />
              ...
              <td>
                <div v-if="withdrawal.status == 'pending'">
                  <span
                    ...
                    class="btn btn-outline-success btn-sm cursor-pointer me-2"
                  >
                ...
                <div v-else-if="withdrawal.status == 'successful'">
                  <span
                    @click="pending(withdrawal.id)"
                    class="btn btn-outline-warning btn-sm cursor-pointer me-2"
                  >
                    Pending
                  </span>
                ...
                <div v-else-if="withdrawal.status == 'failed'">
                  <span
                    @click="approve(withdrawal.id)"
                    class="btn btn-outline-success btn-sm cursor-pointer me-2"
                  >
...
const approve = (id) => {
  form.post(route("admin.withdrawals.approve", id), {
    onBefore: () => confirm("Are you sure you want approve this Transfer?"),
  });
};
const pending = (id) => {
  form.post(route("admin.withdrawals.pending", id), {
    onBefore: () => confirm("Are you sure you want pend this Transfer?"),
  });
};

const decline = (id) => {
  form.post(route("admin.withdrawals.decline", id), {
    onBefore: () => confirm("Are you sure you want decline this Transfer?"),
  });
};
</script>
