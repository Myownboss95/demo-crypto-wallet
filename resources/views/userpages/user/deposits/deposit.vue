          ...
          <div class="row">
            ...
            >
              <div class="border p-4 w-100">
                <img
                  :src="`/storage/payment_methods/${method.svg}`"
                  alt=""
                  class="img-fluid"
          ...
          <div class="px-3">
            <div class="" v-if="!props.validated">
              ...
              <ol>
                ...
                <li>
                  Transfer the amount to the wallet address you scanned or
                  copied.
                </li>
            ...
            <div class="text-center" v-else>
              <div class="placeholder">
                <img
                  :src="`/storage/payment_methods/${method.image}`"
                  alt=""
                  class="w-100 img-fluid"
                />
              </div>
              <strong class="font-size-16 my-1"
                >{{ method.wallet }}
                <span class="ml-2" @click="copy(method.wallet)"
              ...
              <p class="mt-3">
                ...
                equivalent of
                <strong
                  >{{ parseFloat(form.amount) }}{{ method.symbol }} or
                  {{
                    format_money(
                      parseFloat(
                        price[form.name.toLowerCase()]["usd"] * form.amount
                      )
                    )
                  }}</strong
                >
...
const changeAmount = () => {
  ...
  const usdValue = isNaN(parsedAmount)
    ? ""
    : (parsedAmount / price.value[form.name.toLowerCase()]["usd"]).toFixed(4);
...
const changeUSD = () => {
  ...
  const usdValue = isNaN(parsedAmount)
    ? ""
    : (parsedAmount * price.value[form.name.toLowerCase()]["usd"]).toFixed(2);
  form.usd = usdValue;
};
